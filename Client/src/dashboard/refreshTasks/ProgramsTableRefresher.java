                ...
                public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                    ...
                    try (response) {
                        if (response.isSuccessful()) {
                            ...
                            showAlert("Failed to fetch program list: " + response.code() + "\n" + Objects.requireNonNull(response.body()).string(), (Stage) programsTable.getScene().getWindow());
                        }
                    } catch (IOException e) {
                        showAlert("Failed to close the connection properly", (Stage) programsTable.getScene().getWindow());
                    }
                }

                @Override
                public void onFailure(@NotNull Call call, @NotNull IOException e) {
                    showAlert("Failed to get the response from the server, " + e.getMessage(), (Stage) programsTable.getScene().getWindow());
                }
            });
        } else {
            cancel();
